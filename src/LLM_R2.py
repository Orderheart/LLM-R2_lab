...
# Set PATHs
PATH_TO_SENTEVAL = '/home/orderheart/syy/LLM-R2/src/SentEval/SentEval'
PATH_TO_DATA = '/home/orderheart/syy/LLM-R2/src/SentEval/SentEval/data'
# Import SentEval
...
client = OpenAI(
    ...
    api_key="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"
)
...
model_name = 'tpch'
checkpoint = torch.load('/home/orderheart/syy/LLM-R2/src/simcse_models/' + model_name + '/pytorch_model.bin', map_location=torch.device('cpu'))
...
# np.nan_to_num: 用于将数组中的NaN值（不是一个数字）替换为0，
# 同时将正无穷和负无穷值替换为相对较大的有限数值（通常为inf或-inf）
# compute_similarity = lambda s1, s2: np.nan_to_num(cosine(np.nan_to_num(s1), np.nan_to_num(s2)))
compute_similarity = lambda s1, s2: np.nan_to_num(cosine(np.nan_to_num(s1.cpu()), np.nan_to_num(s2.cpu())))
...
def get_promo_meta(db_id, query, rule_path):
    with open('/home/orderheart/syy/LLM-R2/data/data_llmr2/schemas/' + db_id + '.json') as f_sch:
        data = f_sch.read()
...
def get_k_promos(k, pos_pool, neg_pool, db_id, query, logical_plan, method='plan', same=False):
    ...
    if method == 'random':
        ...
        enc2 = enc2.to(device)  # 确保enc2在正确的设备上
        batch1 = [[query]]
        enc1 = batcher(batch1, [db_id])
        enc1 = enc1.repeat((enc2.shape[0], 1)).to(device) # 确保enc1和enc2的形状相同
        sim_scores = []
...
def LLM_R2(dataset, method, num_promos):
    ...
    # df_test = pd.read_csv('/home/orderheart/syy/LLM-R2/data/data_llmr2/queries/queries_' + dataset + '_test.csv').fillna('NA')
    df_test = pd.read_csv('/home/orderheart/syy/LLM-R2/data/data_llmr2/queries/' + 'tpch.csv').fillna('NA')
    promo_pool_pos = get_pool('/home/orderheart/syy/LLM-R2/data/data_llmr2/pools/pos_pool_' + dataset + '_updated.csv', method)
    promo_pool_neg = get_pool('/home/orderheart/syy/LLM-R2/data/data_llmr2/pools/neg_pool_' + dataset + '_updated.csv', method)
    ...
    for index, row in df_test.iterrows():
        if index >= 0:
            ...
            if index % 20 == 0 and index > 0:
                ...
                df_i.to_csv('/home/orderheart/syy/LLM-R2/results/gpt_' + dataset + '_one_promo_' + method + '_updated_to_' + str(index) + '.csv')

                df_t_i = {'demo_time': demo_time_record, 'llm_time': llm_time_record, 'rewriter_time': rewriter_time_record}
                df_t_i = pd.DataFrame(df_t_i)
                df_t_i.to_csv('/home/orderheart/syy/LLM-R2/results/time_gpt_' + dataset + '_one_promo_' + method + '_cleaned_to_' + str(index) + '.csv')
                # break
        # break
    df_gpt['db_id'] = db_ids
    ...
    df_gpt.to_csv('/home/orderheart/syy/LLM-R2/results/gpt_' + dataset + '_one_promo_' + method + '_updated.csv')

    df_t = {'demo_time': demo_time_record, 'llm_time': llm_time_record, 'rewriter_time': rewriter_time_record}
    df_t = pd.DataFrame(df_t)
    df_t.to_csv('/home/orderheart/syy/LLM-R2/results/time_gpt_' + dataset + '_one_promo_' + method + '_cleaned.csv')
