...
# from transformers.utils import is_torch_tpu_available  # Check if still exists in this module
...
# pre_lang_model = SentenceTransformer('/root/LLM-R2/models/')
    ...
    def prepare_features(examples):
        ...
        for idx in range(total):
            if examples[sent0_cname][idx] is None:
                ...
                try:
                    clean_str = examples[sent0_cname][idx].replace('-inf', '-2e308')
                    if clean_str.startswith(("'", '"', '[', '{', '(')) and clean_str.endswith(("'", '"', ']', '}', ')')):
                        examples[sent0_cname][idx] = conv_dict(ast.literal_eval(clean_str))
                    else:
                        examples[sent0_cname][idx] = clean_str
                        # examples[sent0_cname][idx] = conv_dict(ast.literal_eval(clean_str))
                except (ValueError, SyntaxError) as e:
                    print(f"Warning: Failed to parse sent0_cname at index {idx}: {examples[sent0_cname][idx]}")
                    print(f"Error: {e}")
                    examples[sent0_cname][idx] = None  # 或者使用默认值
            ...
            #     examples[sent1_cname][idx] = conv_dict(ast.literal_eval(examples[sent1_cname][idx].replace('-inf', '-2e308')))
                ...
                try:
                    clean_str = examples[sent1_cname][idx].replace('-inf', '-2e308')
                    if clean_str.startswith(("'", '"', '[', '{', '(')) and clean_str.endswith(("'", '"', ']', '}', ')')):
                        examples[sent1_cname][idx] = conv_dict(ast.literal_eval(clean_str))
                    else:
                        examples[sent1_cname][idx] = clean_str
                except (ValueError, SyntaxError):
                    print(f"Warning: Failed to parse sent1_cname at index {idx}. Keeping original string.")
                    examples[sent1_cname][idx] = clean_str
        ...
        if sent2_cname is not None:
            for idx in range(total):
                if examples[sent2_cname][idx] is None:
                    ...
                    try:
                        clean_str = examples[sent2_cname][idx].replace('-inf', '-2e308')
                        if clean_str.startswith(("'", '"', '[', '{', '(')) and clean_str.endswith(("'", '"', ']', '}', ')')):
                            examples[sent2_cname][idx] = conv_dict(ast.literal_eval(clean_str))
                        else:
                            examples[sent2_cname][idx] = clean_str
                    except (ValueError, SyntaxError):
                        print(f"Warning: Failed to parse sent2_cname at index {idx}. Keeping original string.")
                        examples[sent2_cname][idx] = clean_str
