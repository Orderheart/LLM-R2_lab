...
class Similarity(nn.Module):
    ...
    """
    # 温度（Temperature）
    # 温度通常在对比学习中使用，如在 SimCLR 或其他基于对比损失的模型中。温度的作用是平滑或放大相似度的分布。假设相似度结果为 s,引入温度后实际输出为 s/T，其中 T 为温度。
    # T是温度，如果温度很大，相似度趋近于0，如果温度小，相似度误差会被放大
    def __init__(self, temp):
        super().__init__()
        self.temp = temp
        # 沿着最后一个维度（通常是特征维度）上的余弦相似度。
...
class QueryFormerEncoder(nn.Module):
    ...
    def forward(self, attn_bias, rel_pos, x, heights):
        ...
        x = x.to(next(self.mlp_c.parameters()).device)
